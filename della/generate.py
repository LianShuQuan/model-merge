...
if __name__ == "__main__":
    ...
    if args.float32:
        model = AutoModelForCausalLM.from_pretrained(model_name, device_map=args.device , cache_dir=cache_dir)
    else:
        model = AutoModelForCausalLM.from_pretrained(model_name, device_map=args.device , cache_dir=cache_dir, torch_dtype=torch.bfloat16)
    ...
    if dataset_name =="mbpp":
        ...
        if args.full:
            ...
            if args.float32:
                save_gen_results_folder+="_32"
            test_mbpp(args, test_data_path=test_data_path, logger=logger, save_gen_results_folder=save_gen_results_folder, end_index=args.examples)
        ...
        if args.full:
            ...
            if args.float32:
                save_gen_results_folder+="_32"
            test_alpaca_eval(args, finetuned_model_name=model_name, logger=logger, save_gen_results_folder=save_gen_results_folder, end_index=args.examples)
