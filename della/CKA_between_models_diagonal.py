...
if(dataset_name == "alpaca_eval"):
    ...
    test_data_path = "/home/lsq/della/code_data/mbpp.test.jsonl"
...
diagonal_elements = np.diag(selected_matrix)
plt.figure(figsize=(12, 6))

# 绘制柱状图
plt.bar(range(1, len(diagonal_elements) + 1), diagonal_elements)

# 添加标题和轴标签
plt.title(f'CKA Similarity between {model1_name} and {model2_name}_{dataset_name}')
plt.xlabel('Layer')
...
#     os.makedirs(out_dir)
plt.savefig(f'CKA_similarity/cka_similarity_{model1_name}_{model2_name}_{dataset_name}.png')
