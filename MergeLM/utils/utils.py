...
def get_param_names_to_merge_match(input_param_names: List[str], include_param_names_regex: List[str]) -> List[str]:
    """
    Get the names of parameters that need to be merged, including only those that match include_param_names_regex.

    :param input_param_names: List[str], names of input parameters.
    :param include_param_names_regex: List[str], regular expression of names of parameters that need to be included
    :return: List[str], names of parameters to merge after filtering.
    """
    if not input_param_names or not isinstance(input_param_names, list):
        raise ValueError("input_param_names must be a non-empty list.")
    if not include_param_names_regex or not isinstance(include_param_names_regex, list):
        raise ValueError("include_param_names_regex must be a non-empty list.")

    param_names_to_merge = []
    for param_name in input_param_names:
        include = any([re.match(include_pattern, param_name) for include_pattern in include_param_names_regex])
        if include:
            param_names_to_merge.append(param_name)
    return param_names_to_merge
