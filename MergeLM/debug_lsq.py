...
def get_merge_performance(args: argparse.Namespace, finetuned_model_names: list, merge_task_names: list, models_to_merge: list, trainers: list, logger: logging.Logger,
    ...
    if args.layers_to_merge is not None:
        logger.info(f"getting include_param_names_regex... of {args.layers_to_merge}")
        include_param_names_regex = [r"model\.layers\." + str(layer_id) + r"\." for layer_id in args.layers_to_merge]
    ...
    save_model_path = f"./save_merge_models/{'_'.join(merge_task_names)}/{args.save_model_name}_lmhead"
    ...
    if args.merging_method_name == "average_merging":
        ...
        if args.layers_to_merge is not None:
            args.save_model_name = (
                f"deepseek_"
                f"{'instruct' if args.merge_instruct else ''}_"
                f"{'math' if args.merge_math else ''}_"
                f"{'code' if args.merge_code else ''}_"
                f"{args.merging_method_name}_{mask_apply_method_name}_merge_layers_"
                f"{'_'.join(args.layers_to_merge)}"
            )
        else:
            args.save_model_name = (
                f"deepseek_"
