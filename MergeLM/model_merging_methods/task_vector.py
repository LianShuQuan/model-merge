...
from utils.utils import get_param_names_to_merge, get_param_names_to_merge_match
...
class TaskVector:
    def __init__(self, pretrained_model: nn.Module = None, finetuned_model: nn.Module = None, exclude_param_names_regex: list = None, task_vector_param_dict: dict = None,
                 include_param_names_regex: list = None):
        """
        ...
        if task_vector_param_dict is not None:
            ...
            if include_param_names_regex is not None:
                param_names_to_merge = get_param_names_to_merge_match(input_param_names=list(pretrained_param_dict.keys()), include_param_names_regex=include_param_names_regex)
            ...
            with torch.no_grad():
                for param_name in param_names_to_merge:
                    # debugging
                    self.task_vector_param_dict[param_name] = finetuned_param_dict[param_name].cpu() - pretrained_param_dict[param_name].cpu()
    ...
    def combine_with_pretrained_model(self, pretrained_model: nn.Module, scaling_coefficient: float = 1.0):
        ...
        with torch.no_grad():
            ...
            for param_name in self.task_vector_param_dict:
                # debugging
                merged_params[param_name] = pretrained_param_dict[param_name].cpu() + scaling_coefficient * self.task_vector_param_dict[param_name].cpu()
